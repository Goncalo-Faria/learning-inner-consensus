...
class KerasMod(tf.keras.Model):
    def __init__(self, model):
        super(KerasMod,self).__init__()
        self.model = model

    def call(self, inputs):
        # (batch_size, img_rows, img_cols, img_channels)
        shape = inputs.shape
        features = { "images" : inputs, "depth": shape[3], "height": shape[1], "width": shape[2] }
        out , _ = self.model.apply(features)
        return out
...
hpar.model = "CAPSNET"
hpar.verbose = False
hpar.learning_rate = 10
hpar.loss_type = "softmax"
hpar.remake = False
hpar.max_steps = 10

modelraw = CapsuleModel(CapsMLPShared.setup(hpar))

model = KerasMod(modelraw)
...
print(type(data))
opt_fn = tf.compat.v1.train.AdamOptimizer
...
__all__ = ['TfLearner', 'tf_fit', 'tf_loss_batch', 'tf_train_epoch', 'tf_validate', 'tf_get_preds', 'TfOptimWrapper',
           'TfRegularizer', 'tf', 'tf_lr_find', 'TfLRFinder']
...
class TfLearner():
    ...
    def get_preds(self, ds_type: DatasetType = DatasetType.Valid, with_loss: bool = False,
                  n_batch: Optional[int] = None, pbar: Optional[PBar] = None) -> List[Tensor]:
        raise NotImplementedError
...
def tf_lr_find(learn: Learner, start_lr: Floats = 1e-7, end_lr: Floats = 10, num_it: int = 100, stop_div: bool = True,
               **kwargs: Any):
